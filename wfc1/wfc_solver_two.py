...
def wfc_init(prestate):
    ...
    parameters.adjacency_relations = adjacency_extraction_consistent(parameters.wfc_ns, parameters.wfc_ns.patterns)
    ...
    parameters.propagator_matrix = np.zeros((parameters.number_of_directions,
                                             parameters.number_of_patterns,
                                             parameters.number_of_patterns), dtype=np.bool_)
    ...
    output.solving_time = np.full((parameters.rows, parameters.columns), 0, dtype=np.int32)
    output.propagation_time = np.full((parameters.rows, parameters.columns), 0, dtype=np.int32)
    
    parameters.wave_shape = [parameters.rows, parameters.columns, parameters.number_of_patterns]
    ...
    compatible_shape = [parameters.rows, 
                        parameters.columns, 
                        parameters.number_of_patterns, 
                        parameters.number_of_directions]

    WFC_LOGGER.debug(f"compatible shape:{compatible_shape}")
    state.compatible_count = np.full(compatible_shape, parameters.number_of_patterns, dtype=np.int16) # assumes that there are less than 65536 patterns
    ...
    state.starting_entropy = math.log(state.sum_of_weights) - (state.sum_of_weight_log_weights / state.sum_of_weights)
    ...
    output.output_grid = np.full([parameters.rows, parameters.columns], WFC_NULL_VALUE, dtype = np.int64)
    output.partial_output_grid = np.full([parameters.rows, parameters.columns, parameters.number_of_patterns], -9, dtype = np.int64)
    ...
    output.propagation_time = np.full((parameters.rows, parameters.columns), 0, dtype=np.int32)
    output.touch_time = np.full((parameters.rows, parameters.columns), 0, dtype=np.int32)
    output.crystal_time = np.full((parameters.rows, parameters.columns), 0, dtype=np.int32)
    output.method_time = np.full((parameters.rows, parameters.columns), 0, dtype=np.int32)
    output.choices_recording = np.full((parameters.rows, parameters.columns), 0, dtype=np.float32)
...
def wfc_clear(parameters, state, output):
    ...
    pattern_compatible_count = np.fromfunction(pcomp, (parameters.number_of_patterns, parameters.number_of_directions), dtype=np.int16)
    state.compatible_count = np.fromfunction(ccount, (parameters.rows, parameters.columns, parameters.number_of_patterns, parameters.number_of_directions), dtype=np.int16)

    # Likewise, set the weights to their maximum values
    state.sums_of_weights = np.full([parameters.rows, parameters.columns],
                                        state.sum_of_weights, 
                                        dtype = np.float64)
    state.sums_of_weight_log_weights = np.full([parameters.rows, parameters.columns],
                                               state.sum_of_weight_log_weights, 
                                               dtype = np.float64)
    state.entropies = np.full([parameters.rows, parameters.columns],
                                   state.starting_entropy, 
                                   dtype = np.float64)
...
def find_random_unresolved(wfc_state, random_variation):
    ...
    if np.all(np.count_nonzero(wfc_state.wave_table, axis=2) == 1):
        ...
        WFC_LOGGER.info(f"nonzero count: {np.count_nonzero(wfc_state.wave_table, axis=2)}")
...
def check_completion(wfc_parameters, wfc_state):
    ...
    if np.all(np.count_nonzero(wfc_state.wave_table, axis=2) == 1):
        ...
        if (len(pattern_set) != wfc_state.number_of_patterns) and wfc_parameters.wfc_ns.force_use_all_patterns:
...
def finalized_observed_waves(parameters, state, output):
    ...
    for row in range(parameters.rows):
        for column in range(parameters.columns):
            ...
            output.output_grid[row, column] = find_first(True, pattern_flags) # TODO: this line is probably overkill?
...
def wrap_coords(wfc_parameters, cell_coords):
    r = (cell_coords.row + wfc_parameters.wfc_ns.generated_size[0]) % (wfc_parameters.wfc_ns.generated_size[0])
    c = (cell_coords.column + wfc_parameters.wfc_ns.generated_size[1]) % (wfc_parameters.wfc_ns.generated_size[1])
...
def wfc_run(wfc_seed_state, visualize=False, logging=False):
    ...
    while (iterations < wfc_parameters.wfc_ns.iteration_limit) or (0 == wfc_parameters.wfc_ns.iteration_limit):
        ...
        wfc_state, wfc_output = wfc_observe(wfc_state, random_variation, random_number_generator, wfc_parameters, wfc_output)
