...
from wfc.wfc_tiles import load_source_image, image_to_tiles, make_tile_catalog, show_input_to_output, show_extracted_tiles, show_false_color_tile_grid
import wfc.wfc_patterns
from wfc.wfc_patterns import make_pattern_catalog_no_rotations, show_pattern_catalog, make_pattern_catalog_with_symmetry
...
from wfc.wfc_solver import wfc_init, show_wfc_patterns, show_pattern_adjacency, visualize_propagator_matrix, visualize_entropy, wfc_clear, visualize_compatible_count, show_rendered_patterns, render_patterns_to_output, wfc_observe, wfc_partial_output, wrap_coords, show_crystal_time
    ...
    for xnode in xdoc.getroot():
       ...
       if "overlapping" == xnode.tag:
            ...
            wfc_ns = types.SimpleNamespace(
                   output_path="output/",
                   img_filename = "samples/"+ xnode.get('name', "NAME") + ".png", # name of the input file
                   output_file_number = current_output_file_number,
                   operation_name = xnode.get('name', "NAME"),
                   output_filename = "output/" + xnode.get('name', "NAME") + "_" + str(current_output_file_number) + "_" + str(time.time()) + ".png", # name of the output file
                   debug_log_filename = "output/" + xnode.get('name', "NAME") + "_" + str(current_output_file_number) + "_" + str(time.time()) + ".log",
                   seed = 11975, # seed for random generation, can be any number
                   tile_size = int(xnode.get('tile_size', 1)), # size of tile, in pixels
                   pattern_width = int(xnode.get('N', 2)), # Size of the patterns we want. 2x2 is the minimum, larger scales get slower fast.
                   ...
                   generated_size = (int(xnode.get('width', 48)),int(xnode.get('height', 48))), #Size of the final image
            ...
            wfc_ns.channels = wfc_ns.img.shape[-1] # detect if it uses channels other than RGB...
            wfc_ns.tiles = image_to_tiles(wfc_ns.img, wfc_ns.tile_size)
            wfc_ns.tile_catalog, wfc_ns.tile_grid, wfc_ns.code_list, wfc_ns.unique_tiles = make_tile_catalog(wfc_ns)
            wfc_ns.tile_ids = {v:k for k,v in dict(enumerate(wfc_ns.unique_tiles[0])).items()}
            wfc_ns.tile_weights = {a:b for a,b in zip(wfc_ns.unique_tiles[0], wfc_ns.unique_tiles[1])}
            ...
            wfc_ns.pattern_catalog, wfc_ns.pattern_weights, wfc_ns.patterns, wfc_ns.pattern_grid = make_pattern_catalog_with_symmetry(wfc_ns.tile_grid, wfc_ns.pattern_width, wfc_ns.symmetry, wfc_ns.periodic_input)
            if WFC_VISUALIZE:
                show_pattern_catalog(wfc_ns)
            adjacency_relations = adjacency_extraction_consistent(wfc_ns, wfc_ns.patterns)
            if WFC_VISUALIZE:
            ...
            while screenshots_collected < wfc_ns.screenshots:
                ...
                output_filename = "output/" + xnode.get('name', "NAME") + "_" + str(current_output_file_number) + "_" + str(time.time()) + "_" + str(wfc_ns.seed) + ".png", # name of the output file

                profile_filename = "" + str(wfc_ns.output_path) + "setup_" + str(wfc_ns.output_file_number) + "_" + str(wfc_ns.seed) + "_" + str(time.time()) + "_" + str(wfc_ns.seed) + ".profile"
                if WFC_PROFILE:
                ...
                if "minizinc" == solver_to_use:
                    ...
                    if True:
                        ...
                        attempt_wfc_ns.stats_tracking["total_observations_before_success"] = 0
                        ...
                        while attempt_count < wfc_ns.allowed_attempts:
                            ...
                            solution = wfc_run(attempt_wfc_ns, visualize=WFC_VISUALIZE, logging=WFC_LOGGING)
                            solution.wfc_ns.stats_tracking["attempt_count"] = attempt_count
                            solution.wfc_ns.stats_tracking["choices_before_success"] += solution.wfc_ns.stats_tracking["observations"]
                            attempt_wfc_ns.stats_tracking["total_observations_before_success"] += solution.wfc_ns.stats_tracking['total_observations']
                            wfc_logger.info("result: {} is {}".format(attempt_count, solution.result))
                    ...
                    choices_before_success = solution.wfc_ns.stats_tracking["choices_before_success"]
                    time_passed = None
                    if None !=solution.wfc_ns.stats_tracking["time_end"]:
                        time_passed = solution.wfc_ns.stats_tracking["time_end"] - solution.wfc_ns.stats_tracking["time_start"]
                    else:
                        if None !=solution.wfc_ns.stats_tracking["final_time_end"]:
                            time_passed = solution.wfc_ns.stats_tracking["final_time_end"] - solution.wfc_ns.stats_tracking["time_start"]
                ...
                if WFC_VISUALIZE:
                    ...
                    #print(solution)
                    print(1)
                    solution_vis = wfc.wfc_solver.render_recorded_visualization(solution.recorded_vis)
                    #print(solution)
                    print(2)
                    ...
                    print(f"solver recording stack len - {len(solution_vis.solver_recording_stack)}")
                    ...
                    if len(solution_vis.solver_recording_stack) > 0:
                        ...
                        writer = FFMPEG_VideoWriter(video_fn, [solution_vis.solver_recording_stack[0].shape[0], solution_vis.solver_recording_stack[0].shape[1]], 12.0)
