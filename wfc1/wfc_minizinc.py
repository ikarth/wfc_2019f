    ...
    for d_idx, d in enumerate(directions):
        for x in range(shape[0]):
            for y in range(shape[1]):
                ...
                adj_grid[d_idx,y,x] = 1 + (offset_cell[0] + (offset_cell[1] * shape[0]))
    return adj_grid
...
def mz_init(prestate):
    ...
    wfc_state.mz_dzn = {
        ...
        'pattern_names': list(wfc_state.patterns, zip(range(wfc_state.number_of_patterns))),
        "relation_matrix": calculate_adjacency_grid(wfc_state.wfc_ns.generated_size, wfc_state.wfc_ns.adjacency_directions),
        "adjaceny_table": [],
        "adjacency_matrix": []}
